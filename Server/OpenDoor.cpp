...
double OpenDoor::ForceRange[2] {1000,6000};
...
NormalGait::AdjustRcParam OpenDoor::arcParam;
NormalGait::CircleWalkParam OpenDoor::circleWalkParam;
LowpassFilter<6> OpenDoor::filter;
Pipe<OpenDoorParam> OpenDoor::odPipe(true);
std::thread OpenDoor::odThread;
...
int OpenDoor::openDoor(aris::dynamic::Model &model, const aris::dynamic::PlanParamBase &param_in)
{
    ...
    double forceInF[6];
    GeneralFunc::forceInit(param.count,param.ruicong_data->at(0).force[6].fce,forceInF);
    ...
    {
        rt_printf("ODState:%d,DLState:%d,HLState:%d,WTState:%d\n",ODState,DLState,HLState,WTState);
        /*rt_printf("forceRaw:%.2f,%.2f,%.2f,forceInB:%.2f,%.2f,%.2f,forceFiltered:%.2f,%.2f,%.2f\n",
                  param.ruicong_data->at(0).force[6].Fx,param.ruicong_data->at(0).force[6].Fy,param.ruicong_data->at(0).force[6].Fz,
                ODP.forceInB[0],ODP.forceInB[1],ODP.forceInB[2],ODP.forceInB_filtered[0],ODP.forceInB_filtered[1],ODP.forceInB_filtered[2]);*/
   }

    robot.GetPeb(ODP.PebInG,"213");
    robot.GetPee(ODP.PeeInG);
    robot.GetPeb(ODP.PebInMak,beginMak,"213");
    robot.GetPee(ODP.PeeInMak,beginMak);
    odPipe.sendToNrt(ODP);
...
int OpenDoor::locateDoor(Robots::RobotBase &robot, int count)
{
    ...
    case DoorLocateState::Point3:
        ...
        if (fabs(ODP.forceInB_filtered[2])>ForceRange[0] && (count-ODP.countIter)>=1500)
        {
            ...
            DLState=DoorLocateState::PrepareD2H;
            rt_printf("DoorLocate Finished. Going to prepare for adjustRobotD2H\n");
        }
        break;
    case DoorLocateState::PrepareD2H:
        GenerateBodyMotionByFce(robot);
        if(fabs(bodyLastVel[1])<1e-3 && fabs(bodyLastVel[2])<1e-3)
        {
            ODP.countIter=count+1;
...
int OpenDoor::locateHandle(Robots::RobotBase &robot, int count)
{
    ...
    case HandleLocateState::Leftward:
        ...
        if ((fabs(ODP.forceInB_filtered[0])>ForceRange[0] || count-ODP.countIter>3000) && fabs(bodyLastVel[0])<1e-3)
        {
            ODP.countIter=count+1;
    ...
    case HandleLocateState::Follow:
        if(count-ODP.countIter<followCount)
        {
            for(int i=0;i<3;i++)
            {
                ...
                currentPee[6*i+2]=startPeeInB[6*i+2]+(endPeeInB[6*i+2]-startPeeInB[6*i+2])/2*(1-cos((count-ODP.countIter)*PI/followCount));
                //leg 1,3,5
        ...
        else if(count-ODP.countIter<2*followCount)
        {
            for(int i=0;i<3;i++)
            {
                ...
                currentPee[6*i+5]=startPeeInB[6*i+5]+(endPeeInB[6*i+5]-startPeeInB[6*i+5])/2*(1-cos((count-ODP.countIter-followCount)*PI/followCount));
                //leg 0,2,4
...
int OpenDoor::pushHandle(Robots::RobotBase &robot, int count)
{
    ...
    GenerateBodyMotionByFce(robot);
    if (count-ODP.countIter>2500 && fabs(bodyLastVel[2])<1e-3)
    {
        ODP.countIter=count+1;
        return 0;
    }
...
int OpenDoor::pullHandle(Robots::RobotBase &robot, int count)
{
    ...
    GenerateBodyMotionByFce(robot);
    if(count-ODP.countIter>2000 && fabs(bodyLastVel[2])<1e-3)
    {
        ODP.countIter=count+1;
        return 0;
    }
...
void OpenDoor::GetAdjustPull2WParam(Robots::RobotBase &robot)
{
    ...
    aris::dynamic::s_pm_dot_v3(*nowPmb,now2startPebDistInB,now2startPebDistInG);
    rt_printf("PebDistInG:%.4f,%.4f,%.4f;PebDistInB:%.4f,%.4f,%.4f\n",
    now2startPebDistInG[0],now2startPebDistInG[1],now2startPebDistInG[2],
    now2startPebDistInB[0],now2startPebDistInB[1],now2startPebDistInB[2]);
    rt_printf("now2startDist:%.4f,%.4f,%.4f\n",now2startDistance[0],now2startDistance[1],now2startDistance[2]);
}
...
int OpenDoor::pushWalkThrough(Robots::RobotBase &robot, int count)
{
    ...
    case WalkThroughState::AlignWalk:
        ...
        if(ret==0)
        {
	    WTState=WalkThroughState::AdjustWidth;
            ODP.countIter=count+1;

	    arcParam.totalCount=1000;
	    arcParam.isAll=false;
	    std::fill_n(arcParam.actLegs,6,false);
	    arcParam.actLegs[1]=true;
	    arcParam.actLegs[4]=true;
	    arcParam.distance=0.15;
        }
        break;

    case WalkThroughState::AdjustWidth:
	arcParam.count=count-ODP.countIter;
        ret=NormalGait::adjustRc(robot,arcParam);
    ...
    odThread = std::thread([&]()
    {
        ...
        while (1)
        {
            ...
            for (int i=0;i<6;i++)
            {
                ...
                fileGait << param.forceInB_filtered[i] << "  ";
            }
            for (int i=0;i<6;i++)
            {
            ...
            for (int i=0;i<18;i++)
            {
                fileGait << param.PeeInG[i] << "  ";
            }
            for (int i=0;i<6;i++)
            {
                fileGait << param.PebInMak[i] << "  ";
            }
            for (int i=0;i<18;i++)
            {
                fileGait << param.PeeInMak[i] << "  ";
            }
            fileGait << std::endl;
        }
...
<od default="od_param">
    <od_param type="group">
        ...
        <isForward abbreviation="f" type="int" default="1"/>
        <isLeft abbreviation="l" type="int" default="0"/>
        <isJump abbreviation="j" type="int" default="0"/>
    </od_param>
</od>

rs.addCmd("od",OpenDoor::parseOpenDoor,OpenDoor::openDoor);
OpenDoor::recordData();
*/
